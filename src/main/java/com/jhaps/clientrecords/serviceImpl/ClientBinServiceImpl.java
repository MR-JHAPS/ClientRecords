...
import com.jhaps.clientrecords.repository.ClientBinRepository;
...
public class ClientBinServiceImpl implements ClientBinService{
	...
	public ClientBinDto getClientBinById(int clientBinId) {
		...
		return mapper.toClientBinDto(clientBin); // converting to clientBinDto
	}

	@Override
	public Page<ClientBinDto> getAllClientBinClients(Pageable pageable) {
		Page<ClientBin> clientBinList =  clientBinRepo.findAll(pageable);
		return clientBinList.map(mapper::toClientBinDto);
	}
